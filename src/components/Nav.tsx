  ...
  return (
    <header className="sticky top-0 z-50 border-b border-divider bg-bg-primary/90 backdrop-blur-md">
      <div className="mx-auto flex max-w-7xl items-center justify-between px-6 py-4">
        ...
        <nav className="hidden items-center gap-8 lg:flex">
          {NAV_LINKS.map((link) => (
            <a
              key={link.href}
              href={link.href}
              className="text-xs font-medium tracking-[0.2em] text-text-secondary transition-colors hover:text-accent-purple"
            >
              {link.label}
            </a>
          ))}
        ...
        <a
          href="#booking"
          className="hidden rounded-full bg-accent-green px-6 py-2.5 text-xs font-semibold tracking-[0.15em] text-bg-primary transition-all hover:shadow-[0_0_20px_rgba(0,200,83,0.3)] lg:inline-block"
        >
          BOOK A SESSION
        </a>
